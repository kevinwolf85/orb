...
const activityInput = z.object({
  ...
  operationId: z.string().min(1).max(128).optional(), phase: z.enum(["start", "end"]).optional(), source: z.enum(["mcp"]).optional(), parentSessionId: z.string().min(1).max(128).optional(),
}).strict().refine((value) => value.parentSessionId !== value.sessionId, { message: "parentSessionId" });
const activitySchema = {
  ...
  properties: {
    ...
    operationId: { type: "string", minLength: 1, maxLength: 128 }, phase: { type: "string", enum: ["start", "end"] }, source: { type: "string", enum: ["mcp"] }, parentSessionId: { type: "string", minLength: 1, maxLength: 128 },
  },
};

export async function runMcp(): Promise<void> {
  const server = new Server({ name: "orb", version: "0.1.17" }, { capabilities: { tools: {} } });
