...
Each active session gets its own orb. Two to four session orbs gently orbit a shared center, without visible session labels; screen readers still identify each session and state. One session stays large and centered, with Previous and Next controls when more than four sessions are available. Orbital motion pauses with the animation controls, hidden tabs, and reduced motion. Settings also includes **Preview Mode**: selecting a state immediately shows a preview and holds that state. Use **Start auto-cycle** to cycle through states, **Pause auto-cycle** to hold the current state, or **Stop preview** to hide it. Closing settings stops the preview. Cycling pauses when the tab is hidden, animation is paused, or reduced motion is enabled; preview never reports activity or changes live session data.
